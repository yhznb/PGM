...
phenotypeFactor.card=[2,2*ones(1,length(geneCopyVarOneList)),2*ones(1,length(geneCopyVarTwoList))]
...
for inx=1:length(phenotypeFactor.val)
  assignment=assignments(inx,:)
  z=0
  for gene=1:length(geneCopyVarOneList)
    X=zeros(length(geneCopyVarOneList),length(alleleWeights{1}))
    Y=zeros(length(geneCopyVarOneList),length(alleleWeights{1}))
    X(gene,assignment(gene+1))=1
    Y(gene,assignment(gene+1+length(geneCopyVarOneList)))=1
    for i=1:length(alleleWeights{gene})
      z=z+alleleWeights{gene}(i)*(X(gene,i)+Y(gene,i))
    end
  end
  if assignment(1)==1 
     phenotypeFactor.val(inx)=computeSigmoid(z)
   else
     phenotypeFactor.val(inx)=1-computeSigmoid(z)
end
