...
for i=1:phenotypeFactor.card(1)
  for j=1:phenotypeFactor.card(2)
   for k=1:phenotypeFactor.card(3)
     inx=AssignmentToIndex([i,j,k],phenotypeFactor.card)
     if i==1
       phenotypeFactor.val(inx)=alphaList(allelesToGenotypes(j,k))
      else
        phenotypeFactor.val(inx)=1-alphaList(allelesToGenotypes(j,k))
   end
  end 
end
end
